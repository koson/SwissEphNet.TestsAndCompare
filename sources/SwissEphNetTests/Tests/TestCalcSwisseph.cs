...
{
    [Export(typeof(ITest))]
    public class TestCalcSwisseph : BaseTest
    {
        public TestCalcSwisseph()
        {
            Title = "Test planet calculation with SWISSEPH ephemeris";
            Description = "Test the planet calculations with the SWISSEPH ephemeris";
        }

        protected override void RunTest(ResultTestValues result, ISwephProvider provider)
        {
            int iflag = SwephDll.SEFLG_SWIEPH;
